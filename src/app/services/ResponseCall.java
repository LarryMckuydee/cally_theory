...
public class ResponseCall implements Runnable {
    ...
    public ResponseCall(Employee employee, HashMap<Integer,Queue<CallRequest>> queueMap, EmployeeCollection employees) {
    ...
    public void processCall() {
        CallRequest callRequest = queueMap.get(employee.getLevel()).poll();
    ...
    private void failToSolve(CallRequest callRequest, Employee employee) {
        if (employee.getLevel() < 3) {
            ...
            ReceiveCall rc = new ReceiveCall(callRequest, queueMap, employees);
